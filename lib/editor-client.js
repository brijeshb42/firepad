...
firepad.EditorClient = (function () {
  ...
  function OtherClient (id, editorAdapter, name) {
    this.id = id;
  ...
  function EditorClient (serverAdapter, editorAdapter) {
    ...
    this.serverAdapter.registerCallbacks({
      ...
      cursor: function (clientId, cursor, color, clientName) {
        if (self.serverAdapter.userId_ === clientId ||
            !(self.state instanceof Client.Synchronized)) {
          return;
        }
        var client = self.getClientObject(clientId, clientName);
        if (cursor) {
  ...
  EditorClient.prototype.getClientObject = function (clientId, clientName) {
    var client = this.clients[clientId];
    if (client && client.name === clientName) { return client; }
