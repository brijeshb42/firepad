...
class CursorWidget {
  constructor(editor, range, username, color) {
    this.id = username;
    ...
    this._domNode.classList.add('monaco-editor-overlaymessage', 'firepad-remote-cursor');
...
class CursorWidgetController {
  ...
  addCursor(id, position, color) {
    this.cursors[id] = new CursorWidget(this.editor, position, id, color);
  }
  ...
  updateCursor(id, newPos, color) {
    const cursor = this.cursors[id];

    if (cursor) {
      cursor.updatePosition(newPos);
    } else {
      this.addCursor(id, newPos, color);
    }
  }
...
var MonacoAdapter = function () {
  ...
  MonacoAdapter.prototype.setOtherCursor = function setOtherCursor(cursor, color, clientID) {
    ...
    this.cursorController.updateCursor(clientID, range, color);

    /** Clear cursor method */
    return {
      clear: () => {
        otherCursor.decoration = this.monaco.deltaDecorations(otherCursor.decoration, []);
        this.cursorController.removeCursor(clientID);
      }
    };
  };
