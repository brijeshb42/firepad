...
function getConfiguration(editorInstance) {
  ...
  return {
    lineHeight: editorInstance.getOption("lineHeight")
  };
}
class RemoteCursorWidget {
  constructor(codeEditor, widgetId, color, label, tooltipDuration, onDisposed) {
    ...
    this._tooltipNode.style.borderColor = color;
    this._tooltipNode.style.backgroundColor = color;
    this._tooltipNode.style.color = getTextColor(color);
    this._tooltipNode.innerHTML = label;
    this._tooltipNode.style.borderRadius = "2px";
    this._tooltipNode.style.fontSize = "12px";
    this._tooltipNode.style.padding = "5px";
    this._tooltipNode.style.fontWeight = 600;
...
class CursorWidgetController {
  constructor(editor) {
    this.cursors = {};
    this.editor = editor;
    this._tooltipDuration = 50000;
  }

  addCursor(id, clientName, color) {
    this.cursors[id] = new RemoteCursorWidget(this.editor, id, color, clientName, this._tooltipDuration, () => {
  ...
  updateCursor(id, clientName, newPos, color) {
    const cursor = this.cursors[id];
    if (cursor) {
      cursor.setPosition(newPos);
    } else {
      this.addCursor(id, clientName, color);
    }
  }
...
var MonacoAdapter = function () {
  ...
  function MonacoAdapter(monacoInstance) {
    ...
    this.cursorController = new CursorWidgetController(this.monaco);
   ...
   */
  MonacoAdapter.prototype.setOtherCursor = function setOtherCursor(cursor, color, clientID, clientName) {
    /** House Keeping */
    ...
    var otherCursor = this.otherCursors[clientID];
    ...
    if (!otherCursor) {
      ...
      this.otherCursors[clientID] = otherCursor;
    }
    ...
    this.cursorController.updateCursor(clientID, clientName, range, color);
  ...
  MonacoAdapter.prototype.onChange = function onChange(event) {
    ...
    if (this.ignoreChanges || !this.initiated) {
      return;
    }
